...
class BookingController extends Controller
{
    ...
    {
        $cart = Cart::where('userid', Auth::id())->where('satus', 1)->get();
        if ($cart->count()) {
    ...
    {
        return Cartitems::where('ptoductid', $productId)->where('cartid', $cartId)->count();
        # code...
    }
    ...
    public function updatecart(Request $request)
    {
        //  return $request->all();
        //cartid
        $cartId = Cart::where('satus', '1')->where('userid', Auth::id())->get()[0]->cartid;
        //cartitems
        ...
        foreach ($cartItems as $item) {
            ...
            if ($count != "") {
                $stock = Product::where('id', $pid)->select('stock')->get()[0]->stock;

                if ($count != "" && $count <= $stock) {
                    Cartitems::where('cartid', $cartId)->where('ptoductid', $pid)->update(['count' => $request->get($pid)]);
                } else {
    ...
    public function removecart($id)
    {
        ...
        $cartId = Cart::where('satus', '1')->where('userid', Auth::id())->get()[0]->cartid;
        //cartitems
        Cartitems::where('cartid', $cartId)->where('ptoductid', $id)->delete();
    ...
    public function viewcheckout()                              //view checkoutpage
    {
        ...
        foreach ($cartItems as $item) {
            ...
            if ($count != "") {
                $stock = Product::where('id', $pid)->select('stock')->get()->toArray();
            }
    ...
    public function BillingAddress(Request $request) // add Billing address
    {
        ...
        if ($count == 0) {
            ...
            $address->save();
                ...
                $address->uid = Auth::id();
                BillingAddress::where('uid',  Auth::id())->update($request->except('_token', 'total'));
            }
        $total = $request->total;
        $cartId = Cart::where('satus', '1')->where('userid', Auth::id())->get()[0]->cartid;
...
$address = DB::table('address')
   ...
   return view('userpages.payment', ['data' => $data, 'states' => $states, 'address' => $address]);
    ...
    public function paymentDetails(Request $request) 
    {
        ...
        cart::where('cartid', $request->cartid)->update(['satus'=> 2,'bookingdate'=>$dt->toDateString()]);
